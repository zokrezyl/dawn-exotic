#!/usr/bin/env bash
# Build Dawn (webgpu_dawn) for Linux x86_64 in Debug and Release configs.
#
# Why a separate Linux x86_64 build (vs. just consuming google/dawn's CI):
#   The official google/dawn CI workflow does NOT pass -DDAWN_USE_WAYLAND=ON.
#   Dawn's CMakeLists.txt defaults USE_WAYLAND to OFF and only flips
#   USE_X11 ON for UNIX, so the public Dawn-*-ubuntu-latest tarball has
#   DAWN_USE_WAYLAND undefined. The SurfaceSourceWaylandSurface case in
#   src/dawn/native/Surface.cpp is preprocessed out and creating a Wayland
#   WGPUSurface fails with "Unsupported sType" at ValidateSurfaceDescriptor.
#   This script builds the same pinned commit with both X11 and Wayland WSIs
#   enabled so native Wayland sessions work.
#
...
#
# Output:
#   <repo_root>/build-x86_64-debug/src/dawn/native/libwebgpu_dawn.a
#   <repo_root>/build-x86_64-release/src/dawn/native/libwebgpu_dawn.a
#
...
echo "==> dawn-exotic x86_64 build (depot_tools)"
...
for build_type in ${BUILD_TYPES}; do
    lower="${build_type,,}"
    build_dir="${REPO_ROOT}/build-x86_64-${lower}"
    ...
    stage="dawn-linux-x86_64-${lower}-${DAWN_VERSION}"
